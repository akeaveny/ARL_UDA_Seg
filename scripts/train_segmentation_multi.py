...
def train_segmentation_multi(model, target_loader, test_loader, writer):
    ...
    with tqdm(total=config.NUM_STEPS-config.StartIteration, desc=f'Iterations {config.NUM_STEPS}', unit='images') as pbar:
        while i_iter < config.NUM_STEPS:
            ...
            loss_seg1 = criterion(pred_target_aux, labels)
            loss_seg2 = criterion(pred_target_main, labels)
            ...
            pbar.set_postfix(**{'SegLoss: ': loss_seg_value1 + loss_seg_value2})
